...
#' @param checkCov Logical. Whether to check coverage of samples to ensure sufficient coverage is present to estimate age. Default is TRUE
...
#' @param gestationalAge Logical. Whether array-based clocks of gestational age should be calculated. Default is FALSE
#' @param offset Integer. The number of basepairs to add or subtract from your CpG coordinates. Depends on whether the genome assembly of your data was zero-based or one-based. Coordinates used in this function are one-based. Default is 0.
#' @examples
#' madrid(bs, adult.age = 20, fastImpute = FALSE, imp = 5, calcSeqClock = TRUE, calcArrayClocks = TRUE, gestationalAge = FALSE, offset = 0)
#' @import bsseq
#' @import glmnet
#' @import mice
#' @import impute
...
madrid <- function(bs, adult.age = 20, checkCov = TRUE, fastImpute = FALSE, imp = 5, calcSeqClock = TRUE, calcArrayClocks = TRUE, gestationalAge = FALSE, offset = 0) {

    if (calcSeqClock == TRUE) {
        message("[madrid]: will calculate sequencing-based clock (MADRID)")

        if (checkCov == TRUE) {
            message("[madrid]: checking coverage of samples now")
            cov <- bsseq::getCoverage(bs, type = "Cov")
            covMeans <- colMeans(cov, na.rm = TRUE)
            grandCovMean <- mean(covMeans)
            if (grandCovMean < 5) {
                message("[madrid]: WARNING: Grand mean of coverage across all samples is below 5x. Age estimates will likely be inaccurate.")
            }
            if (grandCovMean >= 5) {
               message("[madrid]: Grand mean of coverage across all samples is above 5x. This should be sufficient for age estimates")
            }
        }
        ...
        rownames(meth.mat) <- paste0(seqnames(bs), ":", start(bs) + offset)
    ...
    if ( calcArrayClocks == TRUE) {
        ...
        if (gestationalAge == TRUE) {
            message("[madrid]: estimating gestational age from array-based clocks now")
            data(cpgGA)
            betaFull.subset <- tibble::rownames_to_column(as.data.frame(betaFull.subset), "ProbeID")
            betaFull.subset <- betaFull.subset[match(cpgsGA, betaFull.subset$ProbeID),]
            betaFull.subset$ProbeID <- cpgsGA
            ga <- methylclock::DNAmGA(betaFull.subset)
            ga <- ga[,-1] 
           iAges <- cbind(iAges, ga)
        }
