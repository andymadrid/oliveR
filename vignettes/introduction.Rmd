...
Please note, if your samples happen to be embryonic, you can set the option gestationalAge to TRUE. In that case, the function will calculate the estimated age using several epigenetic clocks trained on prenatal samples. The estimates will be in "gestational age" for each sample, which is weeks of gestation.
...
Telomeres are regions of repetitive DNA sequences harbored at the end of chromosomes. One of the main functions of telomeres is to protect the terminal ends of chromosomes from degredation. However, one thing to note is that the length of telomeres shortens with age which can lead to senescence and apoptosis. As such, shorter telomeres have been linked to increased risk of diseases in humans. A function has been implemented here to estimate telomere lengths from sequencing-based data. Let's take a look at how this would be done.
...
```
The above code is not actually run, again, because the toy dataset we're working with does not contain the necessary CpGs used to estimate telomere length. However, in your dataset, that should not be a problem!
